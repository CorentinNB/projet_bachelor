...
(version with '.-' markers on all plots)
"""
...
def _ivp(rhs, t0, t1, y, ms, rtol, atol, method='auto'):
    if method == 'auto':
        method = 'Radau' if np.abs(y[N:2*N]).max() > 0.5 else 'RK45'
...
def simulate(eps):
    ...
    while t_cur < T - 1e-10:
        ...
        # Store trajectory — decimate to ~8 pts/chunk
...
def plot_sweep(results):
    ...
    fig.suptitle(
        r'N=20 blocks — $\varepsilon$ sweep   '
    ...
    col_titles = [
        r'Relative velocity $\bar{v}_c$',
        r'Relative displacement $\bar{u}_c$',
        r'$\Lambda(t) = \frac{1}{t}\sum\ln\frac{\|\delta_i\|}{\varepsilon_0}$',
        r'State variable $\bar{\Theta}_c$',
    ]
...
if __name__ == '__main__':
    print('ε sweep — N=20 blocks  (Benettin 1980)\n')
    results = {}
    for eps in EPS_LIST:
        print(f'ε = {eps}')
